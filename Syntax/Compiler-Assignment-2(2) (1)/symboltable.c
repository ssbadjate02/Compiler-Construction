...
void assignment(int hash,nodeType *node){
    ...
    if(!strcmp(type,"int")){
        ...
        *x = y;
        //printf("here xx%d",*x);
        symbol_table[hash].value = x;
    }
    else if(!strcmp(type,"float")){
        float y = atof(node->con.value);
        //printf("%d",y);
...
bool check_undeclared_variables(nodeType *node, int scope, int isInFunction, char *functionName)
{
    ...
    if (node->type == typeId)
    {
        //printf("scope undec %s = %d\n %d %s",node->id.name,scope,isInFunction,functionName);
        int isDeclared = 0;

        for (int i = 0; i <= scope; i++)
        {
            // printf("sdgsdhj=%d",fast_string_lookup(node->id.name, i,isInFunction,functionName));
    ...
    if (node->type == typeId)
    {
        ...
        for (int i = 0; i <= scope; i++)
        {
            if (fast_string_lookup(node->id.name, i,isInFunction,functonName))
            {
                ...
                int hash = fast_string_hash(funcScopeName);
                //printf("%s",symbol_table[hash].name);
                strcpy(fargs[type_cnt],symbol_table[hash].name);
                strcpy(typess[type_cnt],symbol_table[hash].type);
                //printf("sdsd%s",node->id.name);
                type_cnt++;
                isDeclared = 1;
            }
        }
...
if (node->type == typeOpr)
{
    ...
    void insert_in_icg(char* op1, char* op2, char op)
    {
    	strcpy(icg[ptr].op1, op1);
    	strcpy(icg[ptr].op2, op2);
    	icg[ptr].op = op;  
    }
     
    char rel_op[] = {EQ_OP, NE_OP, '<', '>', GE_OP, LE_OP};
    char arithmetic_op[] = {'+', '-', '*', '/', '%','&','|'};
    ...
    {	
        ...
        for (int i = 0; i < 1000; i++)
        {
            if (symbol_table[i].len)
                printf("name = %s  type = %s scope = %d isarray = %d functionname=%s\n ", symbol_table[i].name, symbol_table[i].type, symbol_table[i].scope,symbol_table[i].array,symbol_table[i].functionName);
        }
    }
    /*ICG*/
